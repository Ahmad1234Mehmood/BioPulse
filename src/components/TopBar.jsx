    ...
    <>
      <header className="sticky top-0 z-40 flex items-center justify-between px-4 md:px-8 h-16 w-full bg-slate-950/80 backdrop-blur-md border-b border-white/10 shadow-sm font-inter text-sm antialiased">
        <div className="flex items-center gap-4 md:gap-8">
          ...
          <span className="text-xl font-bold tracking-tight text-white">BioPulse AI</span>
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <button
            className="hidden sm:inline text-slate-400 hover:text-indigo-400 transition-colors"
            aria-label="Settings"
            onClick={() => addToast("Settings panel coming soon.", "info")}
          >
          ...
          <button
            onClick={openModal}
            className="hidden sm:block bg-indigo-500 text-white px-4 py-1.5 rounded-lg text-xs font-semibold hover:bg-indigo-400 transition-colors active:scale-95 inner-glow-btn"
          >
            New Project
          </button>
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div
            ...
            onClick={closeModal}
          />
          <div className="relative z-10 w-full max-w-md bg-slate-900 border border-white/10 rounded-2xl shadow-2xl p-6 space-y-5">
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  placeholder="e.g. Airport Security Phase 2"
                  className="w-full bg-slate-950 border border-white/10 rounded-lg py-3 px-4 text-white text-sm focus:ring-1 focus:ring-indigo-500 focus:outline-none placeholder:text-slate-600 transition-all"
                  autoFocus
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <select
                  ...
                  className="w-full bg-slate-950 border border-white/10 rounded-lg py-3 px-4 text-white text-sm focus:ring-1 focus:ring-indigo-500 focus:outline-none transition-all"
                >
                  {PROJ_TYPES.map((t) => (
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-950 border border-white/10 rounded-lg py-3 px-4 text-white text-sm focus:ring-1 focus:ring-indigo-500 focus:outline-none placeholder:text-slate-600 resize-none transition-all"
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-1">
              <button
                onClick={closeModal}
                className="flex-1 py-3 bg-white/5 hover:bg-white/10 border border-white/10 text-white rounded-xl text-sm font-semibold transition-all"
              >
                Cancel
              </button>
              <button
                onClick={handleCreate}
                disabled={!projName.trim() || saving}
                className="flex-1 py-3 bg-indigo-500 hover:bg-indigo-400 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2"
              >
                {saving ? (
                  <>
